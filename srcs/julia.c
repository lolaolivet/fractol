...
static int	init_c(t_data *d)
{
	d->f.c_re = 0.285;
	d->f.c_im = 0.01;
	d->f.x = ((((d->f.col - (W_IMG / 2.0)) * 6.0) / W_IMG) - d->f.dir_x)
		* d->f.zoom;
	d->f.y = ((((d->f.row - (H_IMG / 2.0)) * 6.0) / H_IMG) - d->f.dir_y)
		* d->f.zoom;
	return (0);
}
...
void		draw_julia(t_data *d, int i)
{
	...
	d->f.row = 0;
	d->f.col = 0;
	mlx_put_image_to_window(d->mlx_ptr, d->win_ptr, d->img_ptr, X_IMG, Y_IMG);
}
*/